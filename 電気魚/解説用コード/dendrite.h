...
const int TEND = 300;
const double t= 0.0;
const double TSTART = 0;//ここまで枕詞
const double TPERIOD = 2500;//終了時間

const int NUM = pow(2,8);//ニューロンの数　別に100とか書いてもいい
const int Threads = 128;//ここでは関係なし

const double TAU = 1.0;
const double TAU_recep =10.0;
const double TAU_sinapus =10.0;//Recep-ELLの時定数
const double TAU_sinapus_long =1000.0;//いつか使う
const double TAU_egp =2.0;//EGP内部の時定数
// time constant of neuron (ms)

const double lambda = 30.0;//Recep-ELLのラムダ
const double Iegp_sinapus = 30.0;//EGPも伝わる

const double TH0 = 1.0;//Brandman
const double a = 5.0;
const double b = 1.0;
...
const double w_match = 35.0;//最近接へのW
const double w_plus = w_match*0.8;//隣へのW
const double w_minus = w_match*0.1;//２個となりへのW

const double w_egp =10;
const double w_egp_out = 20;//EGPの抑制の強さ
...
const double I0= 22;
const double I0lif=10;//入力　適宜使い分け
